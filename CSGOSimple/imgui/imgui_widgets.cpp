...
bool ImGui::Button(const char* label, const ImVec2& size_arg)
{
    ...
    bool a = ButtonEx(label, size_arg, 0);
    ImGui::PopStyleVar();
    return a;
}
...
void ImGui::CloseButtonRender(ImGuiID id, const ImVec2& pos, float radius)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + label_size.x + 5);
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + (label_size.x + 34) : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ...
    {
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(RGBA_TO_FLOAT(43, 43, 45, 255)), ImVec4(RGBA_TO_FLOAT(200, 0, 0, 255)), t));
    }
    ...
    {
        RenderText(p, label);
        p = ImVec2(p.x + label_size.x + 10, p.y - 2);
...
#include "..\gui.h"
#include <string>
bool ImGui::Checkbox3(const char* label, bool* v, int* priority)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (*v)
    {
        window->DrawList->AddCircleFilled(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 10), 8, ImColor(24, 155, 208, 255));
        if (g.IO.MouseClicked[1] && (held || hovered))
        {
            if (*priority < 3)
                *priority = *priority + 1;
            else
        ...
        if (*priority == 0)
            window->DrawList->AddText(c_gui.SCALUX, 13, ImVec2(total_bb.Min.x + 6.9, total_bb.Min.y + 5), ImColor(255, 255, 255, 255), "+");
        else
            window->DrawList->AddText(c_gui.VERY_SMALL, 10, ImVec2(total_bb.Min.x + 7, total_bb.Min.y + 6), ImColor(255, 255, 255, 255), std::to_string(*priority).c_str());
    }
    else
    {
        window->DrawList->AddCircleFilled(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 10), 8, ImU32(ImColor(5, 26, 29, 255)), 12);
        window->DrawList->AddText(c_gui.SCALUX, 13, ImVec2(total_bb.Min.x + 6.9, total_bb.Min.y + 5), ImColor(255, 255, 255, 255), "+");
    }
...
ImVec2 ImGui::CalculateTextCenter(const ImVec2& ElementSize, const char* szText) {

    return (ElementSize - ImGui::CalcTextSize(szText)) / 2.0f;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, frame_col, style.FrameRounding, ImDrawCornerFlags_Right);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(160, 3));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 9.77);
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format, 1.f, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    // Draw frame

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(43, 43, 45), 9);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(grab_bb.Min.x, frame_bb.Max.y), ImColor(200, 0, 0), 9);
    // window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5), ImVec2(grab_bb.Min.x, frame_bb.Max.y - 5), ImColor(70, 29, 168), 9);

     // Render grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y), 4, ImColor(180, 10, 10), 360);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 255));
    RenderTextClipped(ImVec2(frame_bb.Max.x - 25, frame_bb.Min.y - 24), ImVec2(frame_bb.Max.x + 5, frame_bb.Max.y - 5), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ImGui::PopStyleColor();
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 3);
    bool a = InputScalar(label, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags);
    ImGui::PopStyleVar();
    return a;
}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 3);
    bool a = InputTextEx(label, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
    ImGui::PopStyleVar();
    return a;
}
